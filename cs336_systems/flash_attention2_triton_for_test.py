...
def flash_fwd_kernel(
    ...
    softmax_scale,
    O_ptr, L_ptr,
    stride_qb, stride_q_seq, stride_qd,
    stride_kb, stride_k_seq, stride_kd,
    stride_vb, stride_v_seq, stride_vd,
    stride_ob, stride_o_seq, stride_od,
    stride_lb, stride_lq,
    BATCH_SIZE,
    ...
    index_batch = tl.program_id(1)       # this indicate which batch to process.

    # get the (seq_len,) block in the Q, K, V by seleting indexing it by batch
    qvk_offset = (index_batch.to(tl.int64) * stride_qb)
    ...
    Q_block_ptr = tl.make_block_ptr(
        ...
        offsets = (block_index_query * BLOCK_SIZE_Q, 0), # Q[batch_size, block_index_query * BLOCK_SIZE_Q, :]
    ...
    l_ptrs = L_ptr + index_batch * SEQ_LEN + offs_q
    tl.store(l_ptrs, m_i)
...
class FlashAttention2(torch.autograd.Function):    
    ...
    def forward(ctx, query: torch.Tensor, key: torch.Tensor, value: torch.Tensor, causal=False):
        ...
        # 并行程序的数量: batch_size * NUM_BLOCKS_Q
        grid = lambda args: (triton.cdiv(seq_len, args["BLOCK_SIZE_Q"]), batch_size, 1)
        
        # L is the logsumexp for the backward pass, one for each query
        L = torch.empty((batch_size, seq_len), device=query.device, dtype=torch.float32)

        flash_fwd_kernel[grid](
            Q_ptr=query, K_ptr=key, V_ptr=value,
            softmax_scale=softmax_scale,
            O_ptr=O, L_ptr=L,
            stride_qb=query.stride(0), stride_q_seq=query.stride(1), stride_qd=query.stride(2),
            stride_kb=key.stride(0),   stride_k_seq=key.stride(1),   stride_kd=key.stride(2),
            stride_vb=value.stride(0), stride_v_seq=value.stride(1), stride_vd=value.stride(2),
            stride_ob=O.stride(0),     stride_o_seq=O.stride(1),     stride_od=O.stride(2),
...
if __name__ == "__main__": 
    ...
    # 测试参数
    B, Q, K, D = 2, 128, 128, 64
    
    query = torch.randn(B, Q, D, device=device, dtype=torch.float32, requires_grad=True)
    key = torch.randn(B, K, D, device=device, dtype=torch.float32)
    value = torch.randn(B, K, D, device=device, dtype=torch.float32)
    
    print(f"Input shapes: query={query.shape}, key={key.shape}, value={value.shape}")
    
    # 调用 Flash Attention (非因果)
    output = FlashAttention2.apply(query, key, value, is_causal=False)
    print(f"Output shape: {output.shape}")
    print(f"Output: {output}")
    
    # 调用 Flash Attention (因果)
    output_causal = FlashAttention2.apply(query, key, value, is_causal=True)
    print(f"Output (causal) shape: {output_causal. shape}")
    print(f"Output (causal): {output_causal}")