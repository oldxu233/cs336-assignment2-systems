...
def main():
    ...
    oom_results = [r for r in results if r['oom']]
    if oom_results:
        first_oom = min(oom_results, key=lambda x: (x['seq_len'], x['d_model']))
        print(f"First OOM configuration: d_model={first_oom['d_model']}, seq_len={first_oom['seq_len']}")
        print(f"Attention memory needed: {first_oom['attention_memory_mb']:.1f} MB")
        
        batch_size = 8
        seq_len = first_oom['seq_len']
        attn_scores_mem = batch_size * seq_len * seq_len * 4 / (1024**2)
        grad_mem = attn_scores_mem * 2
        total_mem = attn_scores_mem + grad_mem
        print(f"Theoretical memory usage:")
        print(f"  Attention scores: {attn_scores_mem:.1f} MB")
        print(f"  Gradients + intermediates: {grad_mem:.1f} MB")
        print(f"  Total: {total_mem:.1f} MB")
    
    print("\nBackward memory savings vs sequence length:")
    print(f"{'seq_len':<10} {'Memory Diff (MB)':<20}")
    print("-" * 30)
